...
# Dictionnaire pour stocker le dernier envoi d'alerte
last_alert_sent = {
    "CPU": None,
    "RAM": None,
    "DISK": None
...
def get_system_status(hostname, role):
    ...
    if cpu_output:
        ...
        if cpu_usage > CPU_THRESHOLD:
            now = datetime.now()
            if last_alert_sent["CPU"] is None or (now - last_alert_sent["CPU"]) > timedelta(hours=1):
                send_alert_email(role, "utilisation CPU", cpu_usage, CPU_THRESHOLD)
                last_alert_sent["CPU"] = now
    ...
    if ram_output:
        ...
        if ram_usage > RAM_THRESHOLD:
            now = datetime.now()
            if last_alert_sent["RAM"] is None or (now - last_alert_sent["RAM"]) > timedelta(hours=1):
                send_alert_email(role, "utilisation RAM", ram_usage, RAM_THRESHOLD)
                last_alert_sent["RAM"] = now
    ...
    if disk_output:
        ...
        if disk_usage > DISK_THRESHOLD:
            now = datetime.now()
            if last_alert_sent["DISK"] is None or (now - last_alert_sent["DISK"]) > timedelta(hours=1):
                send_alert_email(role, "utilisation Disque", disk_usage, DISK_THRESHOLD)
                last_alert_sent["DISK"] = now
